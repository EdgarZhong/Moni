...
- 日常更新不按“操作次数”粗暴加减，必须按**单条交易 old/new 状态贡献差**更新
- 单条交易对某一日期的贡献值只能是 `0` 或 `1`
- `oldContribution = contribution(oldRecord)`
- `newContribution = contribution(newRecord)`
- 同日更新时，`dirtyCountByDate[date] += newContribution - oldContribution`
- 若日期发生变化，则必须对旧日期先减旧贡献、对新日期再加新贡献
- `1 -> 1`、`0 -> 0` 均不得改写计数；`1 -> 0` 才减一；`0 -> 1` 才加一
- 批量操作必须拆成“同一 mutation 中的多条单条贡献更新”，禁止用“这个操作大概会让当天更脏 / 更不脏”之类的经验值近似维护
...
- `queue empty`：当前账本运行时内存 queue 中已无任何待处理日期
- `no consumable tasks in current range`：queue 仍非空，但经当前 `data range` 过滤后，本轮没有可消费日期
- 引擎是否继续 loop / 是否停止，只看“当前是否还能取到新的可消费批次”，**不以 queue 是否为空作为唯一判断条件**
- UI / 日志 / 调试输出必须显式区分以上两种状态，禁止混用成“任务已全部完成”
...
- `data range` 的变化只影响**下一次取批次**；对已经取出并已发往 LLM 的当前 batch **不追溯取消**
...
- **启动语义**：`start consumption` / “开始分类” / “通知消费端启动” 这类入口只允许读取当前 queue 并尝试启动消费者，**绝不允许在 queue 为空时重新扫描账本并补生产任务**

**触发层总原则**：

- 所有条目级数据改写必须在用户确认后、入队前同步完成
- 某次“生产”何时算成功，以“相关数据改写完成 + dirtyDates 已写入运行时内存 queue”为准；不以 `classify_runtime.json` 是否已完成持久化为准
...
1. **队列统一为按天任务视图**：对外业务语义仍只有 `{ date }`；底层持久化主状态改为 `dirtyCountByDate`。
2. **自动触发仅限 CSV 导入**：切换窗口、回前台、重启不自动生成新任务。
3. **重分类由用户操作触发**：标签相关操作与设置页账本区的“全量重分类”在 UI 渐进式确认后生成任务。
4. **前置处理必须同步落盘**：所有解锁与分类元数据联动改写在刷新 `dirtyCountByDate` 前完成。
...
11. **日常维护走条目级增量，不走热路径全账本重算**：`BatchProcessor` 每轮取批次前不得再用全账本扫描补生产；需要恢复或校验时，必须走显式全量重建流程。
