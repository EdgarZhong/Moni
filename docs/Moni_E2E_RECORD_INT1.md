...
1. 直接使用首页现成数据，无需额外注入；首屏可见条目为“西北工业大学云餐便利店”
2. 长按该条目 `560ms` 后停在原位，拖拽面板保持 `Collapsed`，页面文本中未提前出现“停留看细则”
3. 手指下拖到 `y=792` 后，拖拽面板进入 `Expanded`，交易细则区与底部驻留区按预期出现
4. 驻留区描边已统一为主题青色 `rgb(78, 205, 196)`，未再混入红色强调
5. 分类区与底部详情面板继续共享同一个纵向位移量，未再出现额外的第二套位移
6. 分类卡片保持两列半宽布局；`其他` 分类卡实测宽度为 `171px`，与 `居住`、`旅行` 一致，不再独占整行
7. 页面交互阶段 console 未出现新的 runtime error
8. 细则展开阈值现已按父层固定分界线处理，不再依赖子组件进场动画后的 DOM 测量结果

### 8.2 本轮截图取证

- `test-img/drag-panel-collapsed-stable.png`
- `test-img/drag-panel-expanded-cyan-zone.png`

### 8.3 本轮结构化检查结果

1. `Collapsed` 检查：
   - 首页条目中心点：`x=195 / y=532.796875`
   - 长按后 `expandedTextVisible=false`
   - 分类提示 `拖放到分类中` 可见
2. `Expanded` 检查：
   - 下拖目标点：`y=792`
   - `expandedTextVisible=true`
   - 驻留区虚线框：`borderColor=rgb(78, 205, 196)`
3. 阈值边界检查：
   - 视口高度：`844`
   - 固定展开线：`844 - 100 = 744`
   - 长按后原位 `y=530`：`expandedTextVisible=false`
   - 边界线上方 `y=743`：`expandedTextVisible=false`
   - 边界线本身 `y=744`：`expandedTextVisible=true`
4. 两列栅格检查：
   - `居住`：`width=171px`
   - `旅行`：`width=171px`
   - `其他`：`width=171px`
