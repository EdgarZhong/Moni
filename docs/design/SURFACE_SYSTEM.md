...
分类色板的管理原则：
- 分类色与图标分配规则在共享分类视觉模块 `src/ui/shared/categoryVisuals.ts` 中维护，不纳入 tailwind token
- 新增分类时，色彩必须与现有分类可区分，且符合品牌气质（明快、饱和度适中、不刺眼）
- 分类标签的文字色使用该分类色的深色变体，底色使用浅色变体
- 已有默认分类的既有颜色与图标分配属于稳定视觉资产；新的统一分配策略必须自然产出与当前一致的结果，不得表现成两套并行逻辑
- 分类视觉不得再只为默认预置标签手工硬编码；首页、记账页、详情页都必须消费同一套“分类视觉自动分配系统”
- 自动分配系统的输入固定为：当前账本的分类键、分类描述，以及显示层需要的图标变体索引
- 颜色分配必须来自足够大的稳定调色盘；同一账本内每个已分类标签都应获得可复现的专属颜色，不得因为“未知标签”而退回统一灰色
- 图标分配必须先按“分类键 + 分类描述”的语义做正则匹配，再选取对应语义图标组；例如食品相关标签不得落到书本、办公等明显失配的图标组
- 图标组的回退策略也必须区分“已分类但语义未知”与“未分类”：已分类标签可退回中性通用图标组，`?` 只保留给 `未分类 / uncategorized`
- 分类概览横条、条目左侧图标底板、分类标签徽章、拖拽投放格、记账页最近流水参考区，必须共享同一分类视觉解析结果，不得各自维护不同映射表
