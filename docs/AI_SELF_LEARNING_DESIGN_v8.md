...
**用户显式改分类的自动锁定口径（2026-05-06 收敛）**：

- 只要用户显式提交了新的分类意图，系统就应视为一次“确认并冻结当前分类”的动作，默认同步写入 `is_verified = true`
- 这里的“新的分类意图”包括：
  - 从未分类 / 无 `user_category` 状态改到任意有效分类
  - 之前只有 `ai_category`，用户再次显式给出同一个分类（即使分类名与 AI 一致，也属于用户确认）
  - 已存在 `user_category` 时，用户把它改成另一个分类
- 以下情况**不**触发自动锁定：
  - 用户只修改已有 `user_note`（不是从空到非空）
  - 用户再次提交与当前 `user_category` 完全相同的分类（例如对同一 `user_category` 再拖拽一遍）

**`user_note` 与实例库的联动口径（2026-05-06 收敛）**：

- `user_note` 本身也是学习信号；只要用户修改了 `user_note`，系统都应按交易 `id` 同步更新实例库中的当前样本
- 若该交易当前尚未入库，但本次 `user_note` 修改是“从空到非空”，则应先把它视为一次新的确认动作：在存在有效当前分类语义的前提下，自动写入 `is_verified = true`，再同步写入实例库
- 若只是把已有 `user_note` 改成另一段文字，不额外改变锁定状态；但实例库中的对应样本内容仍必须更新到最新值
- 若交易当前仍不存在可学习的有效分类语义（例如最终分类仍为空 / `uncategorized`），则实例库层可不强造无意义样本；一旦后续形成有效分类语义，再按最新 `user_note` 入库

> 设计含义：锁定不是“实例库凑条件”的附属字段，而是“用户已明确确认此分类状态”的系统边界。凡是新的用户分类声明，都默认进入冻结态；没有新的分类声明时，不额外改动锁定状态。
...
export interface SystemPromptConfig {
  language?: string;
  /** 用户自述（全局，语义来自 self_description.md；代码参数名保留 userContext） */
  userContext?: string;
...
| `userContext`   | System Prompt 中的静态文本段 | **语义上对应 Self-Description**，代码参数名保留 `userContext`，来源为 `self_description.md` |
| 记忆快照            | 不存在                   | **新增**，由当前指针指向并注入为 Learned Preferences 段              |
| 实例库             | 不存在                   | **新增**，按固定 rich schema 注入为顶层 `reference_corrections`（含 `rawClass/paymentMethod/transactionStatus/remark`，按完整 `time` 升序，且 `ai_reasoning` / `user_note` 固定存在） |
...
8. If the examples do not reveal any new or changed pattern, return an empty operations array: \`{"operations": []}\`
