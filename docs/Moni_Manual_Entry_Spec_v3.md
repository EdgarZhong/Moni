...
本文件虽然不负责完整记账页 Root 的视觉设计，但当前实现已包含三类直接隶属于记账页的二级层，必须在这里补足其画布边界：

| 场景 | 顶部 header | 底部导航 | 画布占用 | 备注 |
|------|-------------|----------|----------|------|
| 随手记拖拽蒙版 | 无 | 可保留 | 全屏覆盖 | 分类区直接覆盖到整块屏幕，仅底部预留导航高度量级的安全带；不需要因拖拽态主动隐藏底部导航 |
| 压缩包密码输入页 | 无 | 无 | 全屏覆盖 | 顶部要自行处理 safe area，不能共享 Root header |
| 导入指南页 | 无 | 无 | 页面自身全屏覆盖 | 顶部与正文区域由指南页完整接管，并覆盖底部导航，形成真正全屏页 |

补充约束：

- 记账页拖拽蒙版与首页拖拽蒙版属于同一类“全屏拖拽系统”，但记账页版本更简化：
  - 只保留分类区与底部安全带
  - 不再复用首页那套“细则面板父容器区域”
  - 两个浮动提示区固定显示，不承载交互
- 记账页拖拽分类区必须改为**自适应高度 + 最大可视高度限制**；允许内部自然排布，但不允许出现独立滚动条成为主要交互。
- 拖拽态出现时不需要主动隐藏底部导航；只要分类区本身不要压进底部安全带即可。
- 记账页拖拽分类区的标签颜色、底板色与图标必须来自与首页共用的“分类视觉自动分配系统”；不得再只覆盖默认预置分类
- 现有默认分类的既有颜色与图标分配必须在新的统一分配策略下保持兼容；实现上不得拆成“旧标签固定表 / 新标签自动分配”两套并行逻辑
- 自动分配规则固定为：颜色从稳定调色盘生成，图标先按“分类键 + 分类描述”做正则语义匹配，再落到对应图标组；语义未知时回退到中性通用图标，不得使用 `?`
- 导入指南与密码输入页都属于“无全局 header”的二级页；它们顶部的基础留白与真机额外 safe area 必须与设置子页、详情页收口到同一套数值口径。
- 随手记在用户选完分类后，补充详情输入层固定改为“从顶部下拉”的覆盖式表单，不再使用从底部上推的 sheet。
- 软键盘弹出时，不允许推动该输入层整体位移；输入层位置固定，必要时仅允许其内部内容滚动避让键盘。
- 随手记详情输入层的取消手势必须收紧：不允许点击任意遮罩就直接退出；仅允许点击**面板下方、完全不属于面板本体的空白区域**时取消本次输入。
- 面板顶部不再保留“可拖拽 / 可下拉关闭”的横杠提示，避免用户在准备确认或收起键盘时误触退出并丢失已输入内容。
- 随手记详情输入层的字体必须继续区分语义：
  - 用户不可编辑的标题、提示、说明、状态文案使用品牌字体族
  - 用户可编辑的输入内容使用系统 UI 字体
  - 金额输入继续作为例外，保留等宽数字字体
...
- 手记条目的分类图标、分类标签、最近流水参考区中的分类底板，必须与首页共用同一套分类视觉解析结果
- 新增默认标签、后来追加的收入标签、以及用户自定义标签，都必须自动获得图标与颜色；记账页不得因为标签不在旧常量表中而直接隐藏
- `未分类` 才使用问号图标；任何已分类手记条目即使语义未知，也必须显示中性通用图标
