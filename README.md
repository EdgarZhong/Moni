...
- `Moni-UI-Prototype/`：历史 UI 原型存档，仅供追溯参考
- `pixel_bill_backend/`：历史旧逻辑实现与架构参考

它们不再参与当前仓库日常开发；如需追溯历史，可从 `.archive/` 或 Git 历史查看。

当前 UI/UX 表现层设计与实现统一在主仓库内完成；`Moni-UI-Prototype/` 与 `PROTOTYPE_DRIVEN_WORKFLOW_v3.md` 仅保留为历史追溯材料，不再作为现行设计规格入口或实现前置依赖。
...
- 设计规格体系总文档：[DESIGN_SPEC_SYSTEM.md](D:/Code/Moni/DESIGN_SPEC_SYSTEM.md)
- 品牌身份规范：[docs/design/Moni_Brand_Identity.md](D:/Code/Moni/docs/design/Moni_Brand_Identity.md)
- 表面系统规范：[docs/design/SURFACE_SYSTEM.md](D:/Code/Moni/docs/design/SURFACE_SYSTEM.md)
- Design Tokens 唯一代码注册表：[tailwind.config.js](D:/Code/Moni/tailwind.config.js)
...
- 现行表现层规格已切换为 `Brand Identity -> Surface System -> Design Tokens -> Page Spec` 四层体系
...
- 唯一现行表现层实现入口：主仓库 `src/ui/`
- 规格总入口：`DESIGN_SPEC_SYSTEM.md`
