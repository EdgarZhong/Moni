...
本仓库是 Moni 的主仓库，负责承接系统层 / 逻辑层 / 表现层的全部正式实现。

历史参考材料已迁出或归档，不参与日常开发：

- `pixel_bill_backend/`：旧逻辑实现参考，已迁入 `.archive/submodules_2026-04-24/`
- `Moni-UI-Prototype/`：历史 UI 原型，已移至仓库目录外，仅供追溯

UI/UX 表现层设计与实现统一在本仓库 `src/ui/` 内完成。
...
- 协作规则与开发测试原则：[AGENTS.md](./AGENTS.md)
- 项目执行约束与任务看板：[CLAUDE.md](./CLAUDE.md)
- 设计规格体系总文档：[DESIGN_SPEC_SYSTEM.md](./DESIGN_SPEC_SYSTEM.md)
