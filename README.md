...
当前 UI/UX 表现层工作统一在主仓库内完成：

- 表现层探索默认在主仓库 `feature` 分支进行
- 未合入 `main` 的表现层变更视为探索中；合入 `main` 即视为已确认
- 任何表现层变更必须先更新对应 `docs/` 规格文档，再修改主仓库代码
- `Moni-UI-Prototype/` 与 `PROTOTYPE_DRIVEN_WORKFLOW_v3.md` 仅保留为历史材料，不再作为现行工作流入口
...
- 旧 `design/` 工作台已废弃，不再作为 UI/UX 入口；后续表现层探索与实现统一在主仓库内完成
...
- 唯一现行表现层设计与实现入口：主仓库 `src/ui/`
- 规格入口：`docs/` 下对应专项规格文档
- 表现层探索默认在主仓库 `feature` 分支完成，经确认后合入 `main`
- 旧主仓库 `design/` 工作台与 `/__design` 开发态入口已废弃
- `Moni-UI-Prototype/` 与 `PROTOTYPE_DRIVEN_WORKFLOW_v3.md` 当前仅作历史追溯材料
