...
当前 UI/UX 表现层工作重新采用原型驱动开发：

- 原型仓库：`Moni-UI-Prototype`
- 原型仓库远端：`git@github.com:EdgarZhong/Moni-UI-Prototype.git`
- 工作流规范：[PROTOTYPE_DRIVEN_WORKFLOW_v3.md](PROTOTYPE_DRIVEN_WORKFLOW_v3.md)
- 主仓库与原型仓库各自独立运行，主仓库禁止通过 `import`、alias、submodule、npm link 或构建脚本引用原型仓库代码
- 原型仓库是 UI 表现层 source of truth；主仓库只消费经确认的原型规格和复制后的代码
...
- 原型驱动开发工作流 v3：[PROTOTYPE_DRIVEN_WORKFLOW_v3.md](PROTOTYPE_DRIVEN_WORKFLOW_v3.md)
- UI 原型仓库本地工作区：[Moni-UI-Prototype/README.md](Moni-UI-Prototype/README.md)
...
- 旧 `design/` 工作台已废弃，不再作为 UI/UX 入口；后续表现层变更先进入独立原型仓库确认
...
- 唯一表现层设计入口：独立原型仓库 `Moni-UI-Prototype`
- 规范文档：`PROTOTYPE_DRIVEN_WORKFLOW_v3.md`
- 原型启动：进入 `Moni-UI-Prototype/` 后运行 `npm run dev`
- 原型验证：进入 `Moni-UI-Prototype/` 后运行 `npm run typecheck`、`npm run build`
- 旧主仓库 `design/` 工作台与 `/__design` 开发态入口已废弃
- 本项目当前暂不启用 v3 第五章的 Design Scope 导航脚手架；原型仓库维持与主仓库一致的状态驱动导航方式
- 主仓库和原型仓库禁止运行时耦合；如需迁移，只能复制经确认的原型代码/规格并在主仓库内独立接线
