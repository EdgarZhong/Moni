...
- 本轮用户已确认的二级层形态口径：
  ...
  - 导入指南页：无 header、无 footer；整页作为全屏覆盖层接管画布，继续沿用当前正文结构，不再追加“正文整体居中”专项调整任务
...
- 用户已确认：导入指南页不再保留底部导航，必须改成真正接管整块画布的全屏页；实现上要求直接覆盖 Root footer，而不是把正文简单加长到导航栏后面
- 用户已确认：设置页 Android 返回手势现状已收敛为“根页可用，但所有二级页面未正确消费返回事件，仍落到 Root 双击退出逻辑”；本轮需把返回手势接入每个设置子页
- 用户已确认：随手记选完分类后的详情输入层直接改为“从顶部下拉”的覆盖式表单；软键盘弹出时不允许再把整层顶跑
- 用户已确认：详情页原始分类不应上浮到顶部 badge 行；即使去重后只剩一条短字段，也必须保留在原始信息卡片区
- 用户新增真机反馈：压缩包密码输入页字体需与记账页主字体保持一致，不能出现 portal 覆盖页字族回退
- 用户新增真机反馈：首页拖拽分类后的“告诉 AI 为什么”理由弹窗应在出现时自动聚焦输入框，并直接唤起虚拟键盘
- 用户新增交互口径：随手记顶部详情输入层不得再支持“点任意遮罩误退出”；仅允许点击面板下方空白区取消，且顶部横杠移除，避免用户在收起键盘或准备确认时白输一整段内容
- 用户新增字体口径：本轮不做一次性全局大重构，先按补丁方式修正高频输入控件；规则收敛为“所有可编辑控件（input / textarea / select）使用系统字体，静态只读文案继续使用品牌字体，金额输入保留等宽字体”；详情页静态展示不因本轮补丁退回系统字体
...
- 用户已确认：分类视觉重构必须保持“一套统一分配策略”语义，不允许拆成“旧标签固定逻辑 / 新标签自动逻辑”两套路径；要求是现有默认标签在新策略下继续产出当前视觉风格，而不是额外并行分支
...
| Android 返回手势修复 | In Progress / 待真机链路确认 | 第一阶段已切换为正式 `@capacitor/app` 插件接入，并补齐 `AppRoot` 原生日志与 `getBackHandlerDepth()`；`npx cap sync android` 已执行。浏览器侧 `native back` 调试接口与 Playwright 脚本已跑通，JS 返回栈规格已先行验证；当前剩余重点是把设置页所有二级页面正式接入返回栈，并继续确认 Android 系统返回事件是否稳定进入 Capacitor App 插件并到达 JS |
| 0.3.7 真机反馈收口修复 | In Progress | 本轮新增 8 个真机修复点：详情页分类入口点击异常、微信原始分类错误上浮到顶部 badge、设置页二级页返回手势未消费、随手记详情输入层被键盘顶起并改为顶部弹层、导入指南页改为全屏覆盖底部导航、压缩包密码输入页字体回退、首页理由弹窗未自动唤起键盘、随手记详情输入层误触遮罩易退出 |
| 分类视觉自动分配系统重构 | Done / 待 release | 已新增共享分类视觉解析器 `src/ui/shared/categoryVisuals.ts`，首页 / 详情页 / 记账页统一改为消费该注册表；现有默认标签在统一策略下保持既有视觉风格，新增 `收入` 与用户自定义标签可稳定获得颜色、概览色、图标组，`?` 仅保留给未分类；`npm run typecheck` 与 `npm run build` 已通过 |
...
| 全局字体统一治理 | Deferred | 当前只对高频可编辑控件做补丁修正，避免桌面端回退到 serif；完整字体治理与历史覆盖层梳理推后单独收口，本轮不扩展到详情页复制能力 |
