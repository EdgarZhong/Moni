...
- 首页虽然已切到 `AppFacade`，但真实测试账本尚未稳定展示到主舞台
- 当前 Data Range Picker 的 `MIN/MAX` 仍未命中真实账本范围，说明账本加载或范围推导链路仍有问题
- 默认账本产品名已确定为“日常开销”，但该名称只是初始名称，不是不可重命名保留字；默认账本仍必须允许重命名
- 默认账本命名迁移仍需继续完成索引 / 调试链路 / 默认参数的统一收口
- 预算、提示卡、分类概览、流水区虽然已接入读模型边界，但仍未达到原型级数据表现
...
| 账本加载链路排查 | Ready | 当前 `MIN/MAX = 2026-03-10 ~ 2026-04-08` 明显未命中真实账本数据 |
| 首页真实测试数据显示 | Ready | 预算卡、分类概览、提示卡、流水区需由真实测试账本撑起 |
