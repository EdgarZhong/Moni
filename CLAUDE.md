...
| 首页现场 bug 修复收尾 | In Progress | `2026-05-04` 已完成本轮现场 bug 主修复并经你手测确认：`DateRangePicker` 已收口为“草稿态选择，点击确定才提交”；滑块轨道 `MIN/MAX` 固定等于账本真实数据范围；快捷范围与账本范围无交集时，滑块只在边界贴边显示，真正提交给首页过滤与 AI 消费的是显式空区间，不再错误折叠成边界日；快捷按钮区也已从内容宽度驱动改为稳定网格，解决同视口下切换快捷项时的换行抖动。**下一步任务：1）继续冻结日期范围选择器 `requested / visual / applied` 三层语义，避免后续改动再出现“点了就生效 / 取消失效 / 无交集误折叠”；2）按交互域拆分 `MoniHome`，优先把日期范围面板状态、首页列表区、拖拽细则 / 详情态解耦，减少无关状态引发的局部抖动与误重排；3）把根层 AI 控件状态从完整首页读模型里拆成轻量链路，降低跨页控制态不稳。** |
| 全量重分类链路对齐 | In Progress | `2026-05-04` 已完成主链实施并按已确认口径落地：1）分类会话不再按收入 / 支出预先拆流，收入进入同一会话；默认情况下可优先归为“收入”，但退款、撤销、逆向冲回等入账不得机械算作收入；2）`收入` 作为默认普通标签加入分类体系，但不提升为系统特例；只有 `其他` 继续是系统兜底，`uncategorized` 继续只停留在显示 / 运行态；3）`remark` 继续只读展示，用户只编辑“说明 / 理由”，统一写入 `user_note`；仅“改分类”自动锁定，改说明 / 理由不自动锁；4）设置页账本区“全量重分类”已接通三段式链路：先选锁定条目，再做破坏性提交确认，提交完成后再单独确认是否立即通知引擎开始消费；提交动作已统一覆盖分类字段重置、实例库清理与按天入队；5）锁定列表已支持点条目进入对应详情页。**当前剩余事项：1）继续收口全量重分类页面级验证；2）根据现场反馈继续精简确认文案与局部布局；3）把本轮验证结果补回文档。** |
...
1. 全量重分类链路对齐（优先补齐“全账本未锁定交易”样本清理、失败反馈与回归验证）
2. 表现层稳定性收口（优先处理日期范围选择器边界冻结、`MoniHome` 交互域拆分、根层 AI 控件状态独立）
3. Android 文件选择器真机验收
