...
| 首页现场 bug 修复收尾 | In Progress | `2026-05-04` 已完成并经你现场确认 / 复测通过的项：1）`DateRangePicker` 已收口为“草稿态选择，点击确定才提交”；2）滑块轨道 `MIN/MAX` 固定等于账本真实数据范围；3）快捷范围与账本范围无交集时，真正提交给首页过滤与 AI 消费的是显式空区间，不再错误折叠成边界日；4）快捷按钮区已改为稳定网格，解决切换时换行抖动；5）底部导航 AI 流光已按旧版视觉参数恢复，并支持非首页持续显示；6）首页范围状态已改为应用会话内按账本保持，且已修掉刷新后 `DateRangePicker` 快速来回跳的高占用问题；7）AI 工作态高亮日卡已不再用骨架遮住后续真实流水。**当前剩余事项：1）继续修“首页交易列表 / AI 消费范围都必须严格按 data range 与账本范围交集过滤”的显示问题；2）把“刷新 / 重进应用默认应进入本月，但应用未关闭前切页仍保留用户已选范围”这条初始化与会话保持语义彻底收口；3）修“非默认激活账本下，在首页和记账页来回切换时，左上角账本名称瞬间回退到默认账本”的跳变；4）继续冻结 `requested / visual / applied` 三层语义，避免后续改动再把范围选择器状态绕乱。** |
| 全量重分类链路对齐 | In Progress | `2026-05-04` 已完成主链实施并按已确认口径落地：1）分类会话不再按收入 / 支出预先拆流，收入进入同一会话；默认情况下可优先归为“收入”，但退款、撤销、逆向冲回等入账不得机械算作收入；2）`收入` 作为默认普通标签加入分类体系，但不提升为系统特例；只有 `其他` 继续是系统兜底，`uncategorized` 继续只停留在显示 / 运行态；3）`remark` 继续只读展示，用户只编辑“说明 / 理由”，统一写入 `user_note`；仅“改分类”自动锁定，改说明 / 理由不自动锁；4）设置页账本区“全量重分类”已接通三段式链路：先选锁定条目，再做破坏性提交确认，提交完成后再单独确认是否立即通知引擎开始消费；提交动作已统一覆盖分类字段重置、实例库清理与按天入队；5）锁定列表已支持点条目进入对应详情页；6）AI 工作中无法进入全量重分类的现场问题目前已暂时消失，不再作为当前阻塞项保留。**当前剩余事项：1）继续收口全量重分类页面级验证；2）根据现场反馈继续精简确认文案与局部布局；3）把本轮验证结果补回文档。** |
| 表现层稳定性收口（非性能向） | Pending | 基于 `docs/2026-05-04-ui-frontend-audit-report.md` 的前两级优先事项继续推进，重点不是纯性能数字，而是用户可感知的体验稳定性：1）把 `MoniHome` 的主要交互域拆开，降低“修一处抖一处”的连带风险；2）将根层 AI 控件状态独立成轻量 UI 状态链路，避免底部导航出现无关刷新、瞬跳或假状态感；3）把字体与稳定样式上提到入口，并将 `BottomNav` 从首页组件总文件里拆出，减少切页视觉噪音与误伤范围。 |

## 当前优先级

1. 首页现场 bug 修复收尾（优先收掉 data range 交集显示、初始化默认本月、账本名瞬间跳回默认账本）
2. 全量重分类链路对齐（优先补页面级验证与现场反馈回归）
3. 表现层稳定性收口（优先处理日期范围选择器边界冻结、`MoniHome` 交互域拆分、根层 AI 控件状态独立）
4. Android 文件选择器真机验收
...
- `2026-05-04`：截至当前仍待现场继续确认的首页问题有三项：1）首页交易列表是否在所有 range 模式下都严格按交集过滤；2）刷新 / 重进应用后默认是否稳定落在“本月”；3）非默认激活账本下首页与记账页切换时，左上角账本名称是否仍会瞬间闪回默认账本
