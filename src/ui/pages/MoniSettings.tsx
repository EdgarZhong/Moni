...
/**
 * 设置页使用的应用版本展示文案。
 * 这里不要在页面多个位置分别手写版本号，避免 release 后只改了一处又产生视觉口径漂移。
 */
const APP_VERSION_TEXT = "0.4.3";
/**
 * 推理参数面板统一文案。
 * 该文案面向开发者，不再按供应商拆分显示。
 */
const THINKING_HINT_TEXT = "推荐较低推理温度和启用思考（部分模型有效）来增强分类准确性与稳定性。";
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div className="scrollbar-hide" style={{ flex: 1, overflowY: "auto", paddingBottom: 16 }}>
        <SectionCard title="🌐 全局配置">
          ...
          <SettingRow icon="ℹ️" label="关于" desc={`Moni v${APP_VERSION_TEXT}`} onClick={() => onNavigate("about")} />
        </SectionCard>
...
function AIConfigPage({
  ...
  const [maxTokensDraft, setMaxTokensDraft] = useState(String(aiConfig?.maxTokens ?? 8000));
  ...
  useEffect(() => {
    ...
    setMaxTokensDraft(String(aiConfig?.maxTokens ?? 8000));
  ...
  const thinkingSupport = useMemo(() => {
    if (provider === "deepseek") {
      if (/^deepseek-v4-(pro|flash)$/i.test(currentModel)) {
        return {
          enabled: true,
          note: THINKING_HINT_TEXT,
        };
      }
      return {
        enabled: false,
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                style={{
                  ...
                  height: 24,
                  borderRadius: 12,
                  background: thinkingSupport.enabled
                    ? (thinking ? C.mint : C.border)
                    : C.line,
                  position: "relative",
                  cursor: thinkingSupport.enabled ? "pointer" : "not-allowed",
                  opacity: thinkingSupport.enabled ? 1 : 0.72,
                }}
              >
                <div
                  style={{
...
function BudgetPage({
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div ref={scrollRef} className="scrollbar-hide" style={{ flex: 1, overflowY: "auto", padding: "0 16px 24px" }}>
        <FormCard
          title="月度预算"
          footer={
            <Btn
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 32px" }}>
        ...
        <div style={{ marginTop: 28, fontSize: 11, color: C.muted }}>版本 {APP_VERSION_TEXT} · 构建于 2026-05</div>
...
export default function MoniSettings(props: {
  ...
  entryPage?: "root" | "selfDesc" | "budget";
  entryNonce?: number;
}) {
  ...
  useEffect(() => {
    /**
     * 首页提示卡可能要求设置页直接打开某个二级入口。
     * 这里用 entryNonce 作为显式触发器，避免连续两次跳同一页时因为字符串未变化而失效。
     */
    const nextEntryPage = props.entryPage ?? "root";
    setPage(nextEntryPage as SettingsPageKey);
  }, [props.entryNonce, props.entryPage]);
