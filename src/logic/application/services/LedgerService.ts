      ...
      if (!existing) {
        ...
        if (isChanged) {
          updatedRecords[t.id] = {
            ...
            // §2.1 字段：forceUncategorized 时清空，否则保留已有值（兼容无此字段的存量数据）
            ai_confidence: forceUncategorized ? "" : (existing.ai_confidence ?? ""),
            ai_uncertainty_reason: forceUncategorized ? "" : (existing.ai_uncertainty_reason ?? ""),
            ai_used_weak_evidence: forceUncategorized ? false : (existing.ai_used_weak_evidence ?? false),
            ai_evidence_ids: forceUncategorized ? [] : (existing.ai_evidence_ids ?? []),
            ai_needs_review: forceUncategorized ? false : (existing.ai_needs_review ?? false),
